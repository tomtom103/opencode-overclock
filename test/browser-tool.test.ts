...
<html>
  ...
  afterAll(async () => {
    if (sharedBrowserRes) {
      await sharedBrowserRes.dispose?.()
    }
    if (server) {
      server.stop(true)
    }
  ...
  test("executes full sequential browser workflow and diagnostics", async () => {
    ...
    expect(res.tool?.browser).toBeDefined()
    const isolatedTool = res.tool!.browser

    try {
      // 1. Navigate
      const navRes = await callBrowser(isolatedTool, {
        action: "navigate",
        url: serverUrl,
      })
      ...
      // 2. Fill
      const fillRes = await callBrowser(isolatedTool, {
        action: "fill",
        selector: "#name",
        value: "OpenCode",
      })
      ...
      // 3. Select
      const selectRes = await callBrowser(isolatedTool, {
        action: "select",
        selector: "#choice",
        value: "opt2",
      })
      ...
      // 4. Click submit
      const clickRes = await callBrowser(isolatedTool, {
        action: "click",
        selector: "#submit",
      })
      ...
      const domOutput = await callBrowser(isolatedTool, {
        action: "evaluate",
      ...
      // 5. Screenshot
      const shotRes = await callBrowser(isolatedTool, {
        action: "screenshot",
        name: "test-snapshot",
      })
      ...
      const titleRes = await callBrowser(isolatedTool, {
        action: "evaluate",
      ...
      const errClickRes = await callBrowser(isolatedTool, {
        action: "click",
      ...
      // Test action: "console"
      const consoleRes = await callBrowser(isolatedTool, {
        action: "console",
      })
      ...
      const cleanEval = await callBrowser(isolatedTool, {
        action: "evaluate",
        script: "1 + 1",
      })
      ...
      const closeRes = await callBrowser(isolatedTool, {
        action: "close",
      })
  ...
  test("supports tabs and switchTab actions across multiple pages", async () => {
    ...
    try {
      ...
      await callBrowser(tabsTool, { action: "navigate", url: serverUrl })

      // List initial tabs (single tab)
      const tabsInitial = await callBrowser(tabsTool, { action: "tabs" })
      ...
      // Open a second tab
      await callBrowser(tabsTool, {
        action: "evaluate",
        script: `window.open('${serverUrl}/second', '_blank')`,
      })
      await callBrowser(tabsTool, { action: "wait", value: "200" })

      // List tabs with 2 open tabs
      const tabsTwo = await callBrowser(tabsTool, { action: "tabs" })
      ...
      // Switch to tab 2
      const switchRes = await callBrowser(tabsTool, { action: "switchTab", ref: 2 })
      ...
      const tabsAfterSwitch = await callBrowser(tabsTool, { action: "tabs" })
      ...
      const currentTitle = await callBrowser(tabsTool, {
        action: "evaluate",
        script: "document.title",
      })
      ...
      const outOfRange = await callBrowser(tabsTool, { action: "switchTab", ref: 99 })
      expect(outOfRange.toLowerCase()).toContain("out of range")

      // Switch back to tab 1
      const switchBackRes = await callBrowser(tabsTool, { action: "switchTab", ref: 1 })
      expect(switchBackRes).toContain("Switched to tab [1]: Interactive Test Page")
      const backTitle = await callBrowser(tabsTool, {
        action: "evaluate",
        script: "document.title",
      })
  ...
  test("deduplicates repeated console errors in ActionTrace diagnostics", async () => {
    ...
    await callBrowser(browserTool, {
      ...
      script: `
        ...
        btn.textContent = "Trigger Multi Errors";
        btn.onclick = () => {
          for (let i = 0; i < 5; i++) {
